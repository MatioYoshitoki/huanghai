...
Dialog.prototype = {
	...
	getOptions : function( arg ){
		var i,
			...
			defaults = {
				 ...
				 *		header : '弹出层标题',
				 *		content : '弹出层内容',
				 *		yesFn : function(){},	    // OK按钮的回调函数
				 *		noFn : function(){} / true,	// Canal按钮的回调函数
				 *		yesText : 'OK',		    // OK按钮的文本，默认为‘OK’
				 *		noText : 'Canal' 		    // Canal按钮的文本，默认为‘Canal’
				 *	}		
				 */
			};
	...
	createDialogWrap : function( tmpl ){
		...
		var header = tmpl.header ? 
			'<h4 class="easyDialog_title" id="easyDialogTitle"><a href="javascript:void(0)" title="close" class="close_btn" id="closeBtn">&times;</a>' + tmpl.header + '</h4>' :
			'',
			// OK按钮
			...
			noBtn = typeof tmpl.noFn === 'function' || tmpl.noFn === true ? 
				'<button class="btn_normal" id="easyDialogNoBtn">' + ( typeof tmpl.noText === 'string' ? tmpl.noText : 'Canal' ) + '</button>' :
				'',			
			// footer
			footer = yesBtn === '' && noBtn === '' ? '' :
